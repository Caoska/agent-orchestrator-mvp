...
export async function executeConditionalTool(config, context) {
  ...
  // Safe evaluation using expr-eval parser (no code execution)
  let result = false;
  try {
    const expr = parser.parse(rendered);
...
export async function executeTransformTool(config, context) {
  ...
  for (const op of operations) {
    ...
    if (op.type === "template") {
      // Simple string template - merge context with accumulated results
      const templateContext = { ...context, ...result };
      result[op.key] = renderTemplate(op.template, templateContext);
    }
  }
...
export async function executeSendGridTool(config, context) {
  const apiKey = config.api_key || getApiKey(context._workspace, 'sendgrid') || process.env.PLATFORM_SENDGRID_API_KEY;
  if (!apiKey) {
    console.error('SendGrid key check:', {
      hasConfigKey: !!config.api_key,
      hasWorkspace: !!context._workspace,
      hasWorkspaceKey: !!getApiKey(context._workspace, 'sendgrid'),
...
export async function executeLLMTool(config, context) {
  ...
  const apiKey = config.api_key || getApiKey(context._workspace, 'llm') || process.env.LLM_API_KEY;
...
export async function executeTwilioTool(config, context) {
  console.log('Twilio config received:', JSON.stringify(config, null, 2));
  
  const accountSid = config.account_sid || getApiKey(context._workspace, 'twilio', 'account_sid') || process.env.PLATFORM_TWILIO_ACCOUNT_SID;
  const authToken = config.auth_token || getApiKey(context._workspace, 'twilio', 'auth_token') || process.env.PLATFORM_TWILIO_AUTH_TOKEN;
  
  if (!accountSid || !authToken) {
    throw new Error('Twilio credentials required. Add them in Settings or tool config.');
  }
  
  const from = renderTemplate(config.from, context) || process.env.PLATFORM_TWILIO_PHONE_NUMBER;
  const to = renderTemplate(config.to, context);
  const body = renderTemplate(config.body, context);
  
  console.log('Twilio rendered values:', { from, to, body });
  
  if (!from) {
    throw new Error('From phone number required. Set in tool config or add PLATFORM_TWILIO_PHONE_NUMBER.');
  }
  
  const auth = Buffer.from(`${accountSid}:${authToken}`).toString('base64');
  
  const requestBody = new URLSearchParams({ From: from, To: to, Body: body });
