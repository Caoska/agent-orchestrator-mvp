...
export async function executeLLMTool(config, context) {
  ...
  const apiKey = config.api_key || context._workspace?.openai_api_key || process.env.OPENAI_API_KEY;
  if (!apiKey) throw new Error('OpenAI API key required. Add it in Settings or tool config.');
