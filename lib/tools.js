...
export async function executeSendGridTool(config, context) {
  const apiKey = config.api_key || context._workspace?.sendgrid_api_key || process.env.SENDGRID_API_KEY;
  if (!apiKey) throw new Error('SendGrid API key required. Add it in Settings or tool config.');
  
  const to = renderTemplate(config.to, context);
  const from = renderTemplate(config.from, context);
...
export async function executeTwilioTool(config, context) {
  const accountSid = config.account_sid || context._workspace?.twilio_account_sid || process.env.TWILIO_ACCOUNT_SID;
  const authToken = config.auth_token || context._workspace?.twilio_auth_token || process.env.TWILIO_AUTH_TOKEN;
