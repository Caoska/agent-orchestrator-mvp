...
export async function createAgent(agent) {
  if (db) {
    await db.query(
      ...
      [agent.agent_id, agent.project_id, agent.name, JSON.stringify(agent.steps), JSON.stringify(agent.retry_policy), agent.timeout_seconds, agent.webhook_secret, JSON.stringify(agent.trigger), agent.created_at]
    );
  } else {
