  ...
  if (db) {
    await db.query(
      ...
      [
        ...
        usage.platform_emails || 0,  // Only count platform emails
        usage.platform_sms || 0      // Only count platform SMS
      ]
    );
  } else {
    const ws = WORKSPACES.get(workspaceId);
...
export async function createAgent(agent) {
  if (db) {
    await db.query(
      'INSERT INTO agents (agent_id, project_id, name, steps, retry_policy, timeout_seconds, webhook_secret, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)',
      [agent.agent_id, agent.project_id, agent.name, JSON.stringify(agent.steps), JSON.stringify(agent.retry_policy), agent.timeout_seconds, agent.webhook_secret, agent.created_at]
    );
  } else {
