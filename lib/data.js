...
export async function updateWorkspace(workspaceId, updates) {
  ...
  if (db) {
    ...
    console.log('Executing SQL:', `UPDATE workspaces SET ${fields} WHERE workspace_id = $1`, [workspaceId, ...Object.values(updates)]);
    const result = await db.query(
      `UPDATE workspaces SET ${fields} WHERE workspace_id = $1`,
      [workspaceId, ...Object.values(updates)]
    );
    console.log('SQL result:', result.rowCount, 'rows affected');
  } else {
    console.log('No database connection, using in-memory store');
    const ws = WORKSPACES.get(workspaceId);
    if (ws) {
      Object.assign(ws, updates);
      console.log('Updated in-memory workspace:', ws);
    } else {
      console.log('Workspace not found in memory:', workspaceId);
    }
  }
}
