...
export async function initDb() {
  ...
  const schema = `
    ...
    CREATE TABLE IF NOT EXISTS agents (
      ...
      created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
      CONSTRAINT agents_workflow_check CHECK (
        (steps IS NOT NULL AND nodes IS NULL AND connections IS NULL) OR
        (steps IS NULL AND nodes IS NOT NULL AND connections IS NOT NULL)
      )
    );
    ...
    ALTER TABLE runs ADD COLUMN IF NOT EXISTS trigger_type VARCHAR(50) DEFAULT 'manual';
