    ...
    await runQueue.add(
      ...
      {},
      {
        repeat: { 
          ...
          limit: 24 // Only create next 24 instances (1 day ahead)
        },
...
export async function initializeMonthlyReset() {
  try {
    ...
    await runQueue.add(
      ...
      {
        repeat: { 
          pattern: MONTHLY_RESET_CRON,
          limit: 12 // Only create next 12 instances (1 year ahead)
        },
...
export async function scheduleRun(schedule) {
  ...
  if (cron) {
    ...
    console.log(`🔍 REDIS MONITOR: Added cron schedule - schedule_id: ${schedule_id}, agent_id: ${agent_id}, cron: ${cron}`);
    ...
    console.log(`🔍 REDIS MONITOR: Added interval schedule - schedule_id: ${schedule_id}, agent_id: ${agent_id}, interval: ${interval_seconds}s`);
  }
}
