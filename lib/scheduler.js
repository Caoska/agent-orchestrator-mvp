...
export async function cleanupOrphanedJobsForAgent(agent_id) {
  try {
    ...
    for (const job of repeatableJobs) {
      ...
      // Check if this schedule belongs to the missing agent
      const result = await db.query('SELECT agent_id FROM schedules WHERE schedule_id = $1', [schedule_id]);
      
      if (result.rows.length === 0 || result.rows[0].agent_id === agent_id) {
        // This schedule is for the missing agent - remove the repeatable job
        await runQueue.removeRepeatableByKey(job.key);
        console.log(`Removed orphaned repeatable job for agent ${agent_id}: ${job.id}`);
        cleaned++;
        
        // Also clean up the database record if it exists
        if (result.rows.length > 0) {
          await db.query('DELETE FROM schedules WHERE schedule_id = $1', [schedule_id]);
        }
      }
    }
